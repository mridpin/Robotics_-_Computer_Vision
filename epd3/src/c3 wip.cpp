...
int status = 2; // tiene que ser una variable global porque una variable de la clase se llena de basura sin motivo
	...
	switch (status) {
		case 0: // Reorientarse para evitar obstaculo
		{
			...
			if (((theta+PI/4) < (LIMIT*(PI/180))) && ((theta+PI/4) > (-LIMIT*(PI/180)))) {
				...
				status = 1;
				// statusOnePosition = guardamos el 0,0 del robot en odom para usarlo luego
				...
				zero.point.z = 0.0;
				try {
					ROS_INFO("TRANSFORM WORKS?");
					listener.transformPoint("base_link", zero, statusOnePosition); //segfault aqui?? puede que statusOnePosition sea null
					ROS_INFO("TRANSFORM WORKS");
				}catch(tf::TransformException& ex){
		...
		case 1: // Moverse 1 metro para evitar obstaculo
		{
			...
			statusOnePositionRobot.header.stamp = ros::Time();
			try {
			...
			float traveledDistance = sqrt((statusOnePositionRobot.point.x*statusOnePositionRobot.point.x)				+(statusOnePositionRobot.point.y*statusOnePositionRobot.point.y)); // esto devuelve 0, retomar aqui
