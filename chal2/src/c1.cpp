...
void callback(const sensor_msgs::Image::ConstPtr& colorMsg, const sensor_msgs::Image::ConstPtr& depthMsg)
{
	...
	//The depth image comes as a image of 16 bit pixels, encoded as unsigned ints, and with the depth in millimeters
        bDepth = cv_bridge::toCvCopy(depthMsg, sensor_msgs::image_encodings::TYPE_32FC1);
    }
	...
	bDepth->image.convertTo(depth,CV_32F,1.0); //Convert from int to float, and from millimeters to meters
	
	// 1. Procesamiento de imagen  y mapa de profundidad
	processImageAndDepth(color, depth, out_frame);
	//searchTemplate(color, template_image, out_frame);
	...
	if (moment.m00 > MIN_M00) {
		cv::circle(color,cv::Point(x,y),10,cv::Scalar(0,0,255),3);
		cv::ellipse(color, cv::Point(x, y), cv::Size(70,40), 360.0, 0.0, 360.0, cv::Scalar(0,0,255), 3);
...
int main(int argc, char **argv)
{
   ...
   */
    ...
    message_filters::Subscriber<sensor_msgs::Image> sub_image(n, "/camera/rgb/image_rect_color", 1);
